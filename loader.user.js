...
// @author        Rashad Adel
// @namespace     http://tampermonkey.net// @version      1.9.0
...
(function () {
  ...
  // 🔴 خطوة استباقية حرجة: لو الموقع هو qpxpress، بنعمل اعتراض فوري للـ fetch هنا في الـ Loader
  // لمنع مرور أي طلبات قبل تحميل السكريبت الخارجي.
  if (currentURL.includes("qpxpress.com")) {
    window.CUSTOM_PAGE_SIZE = 500;
    if (!window.originalFetch) {
      window.originalFetch = window.fetch;
      window.fetch = async function (...args) {
        // لو السكريبت الخارجي لسه متحملش، بننفذ التعديل من هنا مؤقتاً
        if (window.QP_XPRESS_BYPASS) {
          return window.QP_XPRESS_BYPASS(...args);
        }
        ...
        if (typeof url === "string" && url.includes("page_size=")) {
          url = url.replace(
            /page_size=\d+/,
            `page_size=${window.CUSTOM_PAGE_SIZE}`,
          );
          if (url.includes("page=")) url = url.replace(/page=\d+/, "page=1");
          args[0] = url;
        }
  ...
  // استخدام setTimeout(..., 0) لضمان عدم تعطيل الـ Main Thread أثناء الـ document-start
  setTimeout(() => {
    GM_xmlhttpRequest({
      method: "GET",
      url: CONFIG_URL,
